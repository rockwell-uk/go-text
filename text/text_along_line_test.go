...
func TestLoveHeart(t *testing.T) {
	...
	for name, tt := range tests {
		...
		// heart shape
		heartCoords := generateHeart(tt.heartSize)

		// draw the line
		fillColour := white
		strokeColour := pink
		gc.Translate(centerX, centerY)
		err = textdraw.DrawCoordLine(gc, heartCoords, tt.heartLineWidth, fillColour, tt.heartLineWidth, strokeColour, scale)
		if err != nil {
			t.Fatal(err)
		}
		...
		// text along line
		gc.Translate(0, 0)
		glyphs, err := TextAlongLine(gc, tt.text, heartCoords, typeFace)
		if err != nil {
